...
SELECT *
FROM EMPLOYEES
WHERE LAST_NAME = 'King'; -- 문자열은 작은따옴표(' ')로 표시, 문자열은 대소문자 구분함.
...
-- 정렬할 열이 2개 이상일 때 첫 번째 열로 정렬 후 두 번재 열 정령
SELECT department_id 부서번호, employee_id 사원번호, first_name 이름, last_name 성
FROM employees
ORDER BY department_id, employee_id;

-- 별칭으로 정렬
SELECT department_id 부서번호, last_name 성, salary*12 연봉
FROM employees
ORDER BY 성;
